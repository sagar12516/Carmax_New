...
class data_output():
    ...
    def kinesis_streaming(self,incoming_data):
        AWS_ACCESS_KEY = "AKIASVQKH6JZ3JL3IJV3"
        AWS_SECRET_KEY = "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"
    ...
    def CarData(self,listings, startPt, endPt):
        ...
        for i in range(startPt, endPt):
            ...
            # print('link ', link)
            element.click()
            self.__class__.carInfo['link'].append(link)
            ...
            try:
                ...
                self.__class__.carInfo["pros"].append(ProsLst)
                self.__class__.carInfo["cons"].append(ConsLst)
                ...
                self.__class__.carInfo['reviews'].append(reviews.text)
                #         print("Rating ",starRating.text[0:3])
                self.__class__.carInfo['ratings'].append(starRating.text)
                # print(self.__class__.carInfo)
                # print("Invoking length check.........")
                # self.lengthChk()  # Invoking length check function
                self.driver.back()
            except:
                self.__class__.carInfo['reviews'].append(None)
                self.__class__.carInfo['ratings'].append(None)
                self.__class__.carInfo['pros'].append(None)
                self.__class__.carInfo['cons'].append(None)
                # self.lengthChk()  # Invoking length check function
                self.driver.back()
        self.lengthChk()  # Invoking length check function
        #         self.lengthChk()
